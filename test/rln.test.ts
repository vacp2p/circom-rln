...
describe("Test rln.circom", function () {
  ...
  describe("RFC Compliant Tests - Single Slot Burns", () => {
    ...
    it("Should work with messageId=[5] - arbitrary message_id", async () => {
      const x = genFieldElement();
      const externalNullifier = genFieldElement();
      const selectorUsed = [1, 0, 0, 0]; // Only burn first slot
      ...
      const identitySecret = genFieldElement();
      const userMessageLimit = BigInt(10);
      const messageId = [BigInt(5), BigInt(0), BigInt(0), BigInt(0)]; // Burn message_id 5
      const leaf = calculateLeaf(identitySecret, userMessageLimit);
      const merkleProof = genMerkleProof([leaf], 0);
      const merkleRoot = merkleProof.root;
      ...
      // Verify output matches expected value for message_id=5
      const { y, nullifier } = calculateOutput(
        identitySecret,
        x,
        externalNullifier,
    ...
    it("Should work with messageId=[99] - sparse message_id usage", async () => {
      const x = genFieldElement();
      const externalNullifier = genFieldElement();
      const selectorUsed = [1, 0, 0, 0]; // Only burn first slot

      const identitySecret = genFieldElement();
      const userMessageLimit = BigInt(100);
      const messageId = [BigInt(99), BigInt(0), BigInt(0), BigInt(0)]; // Burn message_id 99
      const leaf = calculateLeaf(identitySecret, userMessageLimit);
      const merkleProof = genMerkleProof([leaf], 0);
      ...
      // Verify output matches RLNv2 formula for message_id=99
      const { y, nullifier } = calculateOutput(
        identitySecret,
        x,
        externalNullifier,
  ...
  describe("Multi-burn Tests with Arbitrary message_ids", () => {
    it("Should burn arbitrary message_ids [5, 10, 15, 20]", async () => {
      const x = genFieldElement();
      const externalNullifier = genFieldElement();
      const selectorUsed = [1, 1, 1, 1]; // All slots active
      ...
      const messageId = [BigInt(5), BigInt(10), BigInt(15), BigInt(20)]; // Arbitrary
      const leaf = calculateLeaf(identitySecret, userMessageLimit);
      const merkleProof = genMerkleProof([leaf], 0);
      const merkleRoot = merkleProof.root;
      ...
      for (let i = 0; i < MAX_OUT; i++) {
        ...
        assert.equal(
          outputY_i,
          y[i],
          `y[${i}] mismatch for messageId ${messageId[i]}`,
        );
        assert.equal(
          outputNullifier_i,
          nullifier[i],
          `nullifier[${i}] mismatch for messageId ${messageId[i]}`,
        );
      }
    });

    it("Should burn sparse message_ids [0, 99, 200, 999]", async () => {
      const x = genFieldElement();
      const externalNullifier = genFieldElement();
      const selectorUsed = [1, 1, 1, 1]; // All slots active

      const identitySecret = genFieldElement();
      const userMessageLimit = BigInt(1000);
      const messageId = [BigInt(0), BigInt(99), BigInt(200), BigInt(999)]; // Very sparse
      const leaf = calculateLeaf(identitySecret, userMessageLimit);
      const merkleProof = genMerkleProof([leaf], 0);
      ...
      for (let i = 0; i < MAX_OUT; i++) {
        ...
        assert.equal(outputY_i, y[i]);
        assert.equal(outputNullifier_i, nullifier[i]);
      }
    });

    it("Should fail if any active messageId >= userMessageLimit", async () => {
      const x = genFieldElement();
      const externalNullifier = genFieldElement();
      const selectorUsed = [1, 1, 1, 0]; // First 3 active

      const identitySecret = genFieldElement();
      const userMessageLimit = BigInt(15);
      const messageId = [BigInt(5), BigInt(10), BigInt(20), BigInt(0)]; // messageId[2]=20 >= 15
      const leaf = calculateLeaf(identitySecret, userMessageLimit);
      const merkleProof = genMerkleProof([leaf], 0);
  ...
  describe("Non-contiguous Selector Tests (RFC Feature)", () => {
    ...
    it("Should burn alternating slots [1,0,1,0] with invalid inactive messageIds", async () => {
      const x = genFieldElement();
      const externalNullifier = genFieldElement();
      const selectorUsed = [1, 0, 1, 0]; // Alternating

      const identitySecret = genFieldElement();
      const userMessageLimit = BigInt(10);
      // Inactive slots have invalid messageIds (>= userMessageLimit)
      const messageId = [BigInt(5), BigInt(999), BigInt(8), BigInt(9999)];
      const leaf = calculateLeaf(identitySecret, userMessageLimit);
      const merkleProof = genMerkleProof([leaf], 0);
      ...
      // Should succeed - inactive slots skip range check
      const witness: bigint[] = await circuit.calculateWitness(inputs, true);
      await circuit.checkConstraints(witness);
  ...
  describe("Nullifier Uniqueness", () => {
    ...
    it("Should produce NULL nullifiers (zero) for inactive slots regardless of messageId", async () => {
      const x = genFieldElement();
      const externalNullifier = genFieldElement();
      const selectorUsed = [1, 0, 0, 0]; // Only first active

      const identitySecret = genFieldElement();
      const userMessageLimit = BigInt(100);
      // Different messageIds in inactive slots
      const messageId1 = [BigInt(5), BigInt(10), BigInt(20), BigInt(30)];
      const messageId2 = [BigInt(5), BigInt(99), BigInt(50), BigInt(70)];
      const leaf = calculateLeaf(identitySecret, userMessageLimit);
      const merkleProof = genMerkleProof([leaf], 0);
      ...
      // All inactive slot nullifiers should be zero
      for (let i = 1; i < MAX_OUT; i++) {
        const nullifier1_i = await getSignalArray(
          circuit,
          witness1,
          "nullifier",
          i,
        );
        const nullifier2_i = await getSignalArray(
          circuit,
          witness2,
          "nullifier",
          i,
        );
        assert.equal(nullifier1_i, BigInt(0), `nullifier[${i}] should be NULL`);
        assert.equal(nullifier2_i, BigInt(0), `nullifier[${i}] should be NULL`);
      }
    });
  });
    ...
    it("Should measure zkSNARK proof generation time with 1 message burn", async function () {
      ...
      for (let i = 0; i < numRuns; i++) {
        ...
        const { proof, publicSignals } = await snarkjs.groth16.fullProve(
          inputs,
          path.join(__dirname, "..", "build", "rln_js", "rln.wasm"),
          path.join(__dirname, "..", "zkeyFiles", "rln", "final.zkey"),
        );
